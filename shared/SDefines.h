...
// tree node struct for random forest
...
typedef struct ProjectInfo
    ...
    std::vector<knnType>knn_labels;
    // used in random forest training <features + label>, the last item is class number(label)
    //std::vector<double*>ran_sample;
    std::vector<double> knnVectorPtr;
    int knnCounter;
    int topK;
    RFNode *root;
	#endif	// defined multispec_lin
...
typedef struct WindowInfo
	...
	UInt32					numberImageOverlays;
			...
			// The number of overlays for this window. The indices are in the 
